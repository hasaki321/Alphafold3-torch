...
import jax
@jax.jit
def convert_to_jax(arr):
    return jax.device_put(jnp.array(arr))
from collections import OrderedDict
def transform_model_haiku_params(model_dir: pathlib.Path) -> hk.Params:
  """Get the Haiku parameters from a model name."""
  params: OrderedDict[str, OrderedDict[str, jnp.Array]] = OrderedDict()
  save_path = "./jax_weights/model_cache.pkl"
  if os.path.exists(save_path):
    with open(save_path, "rb") as f:
      params = pickle.load(f)
      return params
  else:
    os.makedirs('./jax_weights',exist_ok=True)
  ...
  with open_for_reading(model_files, is_compressed) as stream:
    for scope, name, arr in read_records(stream):
      if scope.endswith('trunk_pairformer/pair_attention2/v_projection') or scope.endswith('trunk_pairformer/single_attention_v_projection') or scope.endswith('trunk_pairformer/pair_attention1/v_projection') or scope.endswith('transformerv_projection'):
        if len(arr.shape)==4 or len(arr.shape)==5:
          new_arr = jnp.einsum('...ahi->...hia',arr)
          params.setdefault(scope, OrderedDict())[name] = jnp.array(arr)
        else:
          raise ValueError(f'shape of {scope} !=4 or 5')
        print(f'convert {scope} from shape {arr.shape} to {new_arr.shape}')
      else:
        # if scope.endswith('v_projection'):
        #   print(scope)
        params.setdefault(scope, OrderedDict())[name] = jnp.array(arr)
  if not params:
...
if __name__ =='__main__':
  transform_model_haiku_params(pathlib.Path('./models'))