...
alignment = {
    ...
    'distogram':['diffuser/distogram_head'], 'half_logits':['half_logits'],
    ...
    'experimentally_resolved_logits':['experimentally_resolved_logits/weights'],
    'left_distance_logits':['left_half_distance_logits'],
    'plddt_logits':['plddt_logits/weights'],
...
def post_process(jax_name:str, block_index_list:list):
    ...
    if '/transition_block/' in jax_name:
        ...
        if 'diffusion_atom_transformer_encoder' in jax_name[:pos]:
            ...
            jax_name = jax_name.replace('transition_block','transformerffw_###')
    ...
    if '/single_attention/' in jax_name:
        ...
        elif 'diffuser/confidence_head/__layer_stack_no_per_layer' in jax_name:
            jax_name = jax_name.replace('/single_attention/','/single_attention_')
        else:
            raise ValueError(f'not match prefix found in {jax_name}. It should have diffuser/evoformer/__layer_stack_no_per_layer or diffuser/confidence_head/__layer_stack_no_per_layer')
    ...
    replace_dict = {
        ...
        'ffw_ffw_':'ffw_',
        'diffuser/~/diffusion_head/transformer/pair_logits_projection':'diffuser/~/diffusion_head/transformer/__layer_stack_with_per_layer/pair_logits_projection',
    }
...
with open('load_params/torch_state_dict.log','r') as f:
    for line in f.readlines():
        if line.strip():
            ...
            if torch_name.split(' ')[0] =='diffusion_head.diffusion_atom_cross_att_encoder.embed_pair_distances.weight':
                jax_name = 'diffuser/~/diffusion_head/diffusion_embed_pair_distances_1/weights'
            if torch_name.split(' ')[0] == 'diffusion_head.diffusion_atom_cross_att_encoder.embed_pair_offsets.weight':
                jax_name = 'diffuser/~/diffusion_head/diffusion_embed_pair_offsets_1/weights'
            if torch_name.split(' ')[0] == 'diffusion_head.diffusion_atom_cross_att_encoder.single_to_pair_cond_col.weight':
                jax_name = 'diffuser/~/diffusion_head/diffusion_single_to_pair_cond_col_1/weights'
            if torch_name.split(' ')[0] == 'diffusion_head.diffusion_atom_cross_att_encoder.single_to_pair_cond_row.weight':
                jax_name = 'diffuser/~/diffusion_head/diffusion_single_to_pair_cond_row_1/weights'
