...
_BUCKETS = flags.DEFINE_list(
    'buckets',
    # pyformat: disable
    ['37', '512', '768', '1024', '1280', '1536', '2048', '2560', '3072',
     '3584', '4096', '4608', '5120'],
    # pyformat: enable
...
def torch_tree_map(func, pytree):
    ...
    # elif isinstance(pytree, Sequence) and not isinstance(pytree, str):
    #     # If pytree is a list or tuple, apply func to each element
    #     return type(pytree)(torch_tree_map(func, item) for item in pytree)
    else:
...
class ModelRunner:
  ...
  def model_params(self):
    ...
    return params_th.get_model_torch_params(model_dir=self._model_dir)
  
  def run_inference(
      self, featurised_example: features.BatchDict, rng_key: jnp.ndarray
  ) -> model_th.ModelResult:
    """Computes a forward pass of the model on a featurised example."""
    # featurised_example_jax = jax.device_put(
  ...
  def print_model_parameters(model, prefix=""):
    ...
    for name, module in model.named_children():
        ModelRunner.print_model_parameters(module, prefix=f"{prefix}{name}.")
...
def predict_structure(
  ...
  for seed, example in zip(fold_input.rng_seeds, featurised_examples):
    ...
    model_runner.print_params()
    model_runner.print_model_parameters(model_runner._model)
    print('model.state_dict')
    for key,value in model_runner._model.state_dict().items():
...
def main(_):
  ...
  fold_inputs = iter(sorted(fold_inputs, key=lambda x:int(x.name.split("aa_")[0])))
  # print(next(fold_inputs).name)
